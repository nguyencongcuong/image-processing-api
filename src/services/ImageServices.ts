...
class ImageServices {
	...
	private onCacheInit() {
		fs.mkdir(
			...
			(error) => {
				if (error) {
					log.error('ImageServices.onCacheInit()', 'Unable to create .cache folder!');
				}
				log.info('ImageServices.onCacheInit()', '.cache folder created successfully!');
			},
		);
	}
	...
	public async resize(params: ResizeServiceModel): Promise<void> {
		try {
			...
			log.info('ImageServices.resize()', 'The image has been resized successfully!')
	...
	public isResized(params: CacheServiceModel) : boolean {
		const data = fs.readFileSync(PATH.CACHE_FOLDER + '/resized-images.txt', { flag: 'a+'}).toString()
		const bool = data.includes(`${params.name}-${params.width}x${params.height}.jpg`)
		if(bool) {
